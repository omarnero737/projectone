$(()=>{
    ...
    $.post("http://localhost:8080/ajaxproject/ajax.php", {name : name, time: time , type: type} ,(one,two,three)=>{
    ...
    const task = {time : time,type : type,target: target};
    $.post("http://localhost:8080/ajaxproject/ajax.php", {name:type, time:time, type:'main'},(one)=>{
        document.querySelector('.click').addEventListener('click',()=>{
            document.querySelector('.data').innerHTML = one;
    })
});
